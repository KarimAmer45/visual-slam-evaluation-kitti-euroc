...
pip install -e .
slam-eval demo --output-dir docs/results
cat docs/results/  # trajectory and ATE plots
```

To evaluate an ORB-SLAM3 run on EuRoC MH_01:

```bash
slam-eval eval \
  --estimate ORB_SLAM3/CameraTrajectory.txt \
  --ground-truth data/euroc/MH_01_easy/mav0/state_groundtruth_estimate0/data.csv \
  --ground-truth-format euroc --with-scale --plot-dir outputs/orbslam3_mh01
```
